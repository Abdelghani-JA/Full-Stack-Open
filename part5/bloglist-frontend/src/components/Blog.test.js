...
test('blog renders just the blog\'s title and author by default', async () => {
  ...
  const { container } = render(<Blog blog={blogExample} user={user} getBlogs={() => jest.fn()} />)
...
/* since the handleLikes is not received as props,
  we have to add onclick event for the sake of this test,
   ...
   calling the orginal function will trigger a network requests !
*/
...
test('form calls the event handler it received as props with the right details when a new blog is created', async () => {
  ...
  const newBlog = { title:'Using Jest for react apps', author:'me', url:'https://jestjs.io/' }
